...
def analyze():
     ...
     since = datetime.now() - timedelta(days=30)
     logs = list(db.auditlogs.find({"userId": ObjectId(user_id), "createdAt": {"$gte": since}}))
...
# delete anomaly
         if action == "delete":
             count = sum(1 for l in logs if l.get("action") == "delete" and l["createdAt"].date() == today)
             print(f"delete count today: {count}, avgDel: {baseline.get('avgDel')}, stdDel: {baseline.get('stdDel')}")
             d_score = Zscore(count, baseline["avgDel"], baseline["stdDel"])
             if d_score > 0.5:
                 details.append(f"mass deletion: {count} today (avg {baseline['avgDel']:.1f}/day)")
                 score += d_score
